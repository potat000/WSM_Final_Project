...
def _build_prompt(language, doc_text_truncated, chunk_text, metadata):
    """
    純粹負責構建 Prompt，不涉及 API 呼叫
    """
    subject_name = ""
    if metadata:
...
async def _generate_context_async(client, model, prompt, semaphore):
    """
    非同步調用 Ollama API
    semaphore 用於限制同時發送的請求數量，避免灌爆 Server
    """
...
async def _process_doc_chunks_async(doc_chunks, full_doc_text, language, doc_metadata, ollama_config, concurrency=4):
    """
    處理單一文檔的所有 Chunks，並行生成 Context
    """
    client = AsyncClient(host=ollama_config["host"])
    semaphore = asyncio.Semaphore(concurrency) # 限制並發數
    
    tasks = []
    
    # 截斷 doc_text (只需做一次)
    doc_text_truncated = full_doc_text[:6000]

    for chunk_text in doc_chunks:
        prompt = _build_prompt(language, doc_text_truncated, chunk_text, doc_metadata)
        
        # 建立異步任務
        task = _generate_context_async(client, ollama_config["model"], prompt, semaphore)
        tasks.append(task)

    # 並行執行並等待結果
    # 使用 tqdm_asyncio 顯示進度條
    contexts = await tqdm_asyncio.gather(*tasks, desc="Generating Contexts (Async)")
    
    return contexts

def chunk_documents(docs, language, chunk_size=1000, chunk_overlap=200):
    """
    主入口函數：負責分塊並調用 Async 處理流程
    """
    ...
    print(f"開始處理文檔 (Language: {language})... 使用異步並發加速")

    # 為了讓代碼結構清晰，我們這裡還是逐個文檔處理
    # (如果要極致速度，可以把所有文檔的所有 chunk 攤平一次跑，但會失去文檔邊界較難管理)
    for doc_index, doc in enumerate(docs):
        if 'content' not in doc or not isinstance(doc['content'], str): continue
        if 'language' not in doc: continue
        if doc['language'] != language: continue
        
        full_doc_text = doc['content']
        doc_chunks = text_splitter.split_text(full_doc_text)
        
        print(f"正在處理文檔 {doc_index + 1}/{len(docs)}: {len(doc_chunks)} 個 Chunks")

        # --- 核心修改：呼叫 Async 函數 ---
        # 這裡使用 asyncio.run 來執行異步任務
        # 設定 concurrency=4 (請確保您的 OLLAMA_NUM_PARALLEL 也有設定)
        try:
            contexts = asyncio.run(_process_doc_chunks_async(
                doc_chunks, full_doc_text, language, doc, ollama_config, concurrency=4
            ))
        except Exception as e:
            print(f"Async processing failed: {e}")
            contexts = [""] * len(doc_chunks) # 失敗回退
